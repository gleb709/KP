...
public class UserAccountInfoCommand implements ActionCommand {
    @Override
    public String execute(HttpServletRequest request) {
        ...
        String contractNumber = request.getParameter(RequestParameter.CONTRACT_NUMBER);
        try {
            Optional<User> user = userService.findUserInfoByContractNumber(contractNumber);
            if (user.isPresent()) {
                session.setAttribute(RequestParameter.LOGIN, user.get().getAccount().getLogin());
                ...
                session.setAttribute(RequestParameter.ACCOUNT_STATUS, user.get().getAccount().getStatus());
                page = PagePath.USER_ACCOUNT_INFO.getPagePath();
            } else {
